...
data_dir <- "C:\\Users\\Nick\\Documents\\Biomo\\Glenbeigh Data\\11_A030E2_A1"
...
#ibi <- read.empatica.ibi(file)

#create a data frame of the ibi data
#ibi_frame <- data.frame(
 # matrix(unlist(ibi), ncol=length(ibi))
#  )

#Reorder the columns so time is X1
#ibi_frame <- ibi_frame[,c(2,1)]

#Name the columns
#names(ibi_frame)[1] <- "Time"
#names(ibi_frame)[2] <- "ibi_data"

#Save the ibi data as an ASCII file
#write.table(ibi_frame$ibi_data, "ibi_ascii.txt", sep = "    ", row.names = FALSE,col.names = FALSE)

#Use the RHRV library to process the data

#Create the HRV data structure
#HRV_Data <- CreateHRVData(Verbose = TRUE)

#Load the ascii ibi file
#HRV_Data <- LoadBeatRR(
  #HRVData = HRV_Data,
  #RecordName = "ibi_ascii.txt",
  #scale = 1
  #)

#Build the Non interpolated heart rate
...
#Filter out artifacts, uses sensible paramters if not specified
...
#Can also manually filter
#EditNIHR(HRV_Data)

#Save to PDF
#png("NIHR_filtered.png")
#PlotNIHR(HRV_Data)
#dev.off()

#To do any frequency analysis, heart beats events must be evenly spaced, but they are not.
#They occur whenever they occur, almost always unevely.
#Interpolating the NIHR will evenly space them for frequency analysis
...
#Frequency Analysis
#Prepare the data structure, multiple can be done, which can be indexed in the other functions and stored in the data structure
#HRV_Data <- CreateFreqAnalysis(HRV_Data)

#Power Band Analysis
#HRV_Data <- CalculatePowerBand(HRV_Data, 
 #                              indexFreqAnalysis = 1,    #Index of the analysis created in the previous line
  #                             size = 300,               #Size of the window where the parameters are calculated
   #                            shift = 30,               #shift the window by this many seconds at a time
    #                           sizesp = 2048,            #Number of points for calculating the fourier transform
     #                          type = "fourier",         #Fourier transform is the default
      #                         ULFmin = 0,               #Mins and maxes for the bands, set to default
       #                        ULFmax = 0.03, 
        #                       VLFmin = 0.03, 
         #                      VLFmax = 0.05, 
          #                     LFmin = 0,05, 
           #                    LFmax = 0.15, 
            #                   HFmin = 0.15, 
             #                  HFmax = 0.4)
#Save to PDF
#PlotPowerBand(HRV_Data, indexFreqAnalysis = 1)
#dev.off()

#A second frequency analysis with a wavelet transform
#HRV_Data <- CreateFreqAnalysis(HRV_Data)
#HRV_Data <- CalculatePowerBand(HRV_Data, 
 #                              indexFreqAnalysis = 2,
  #                             type = "wavelet",
   #                            wavelet = "la8",          #Default "less assymmetric 8" methodlogy
    #                           bandtolerance = 0.01,     #in Hertz
     #                          relative = FALSE,         #If true, the band tolerance would be a percentage
      #                         ULFmin = 0,
       #                        ULFmax = 0.03, 
        #                       VLFmin = 0.03, 
         #                      VLFmax = 0.05, 
...
#Save to PDF
#png("wavelet.png")
#PlotPowerBand(HRV_Data, indexFreqAnalysis = 2)
#dev.off()